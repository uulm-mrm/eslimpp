...
weighted_types_harmony = [
    sl.WeightedTypes(sl.RelationType.CONFLICT, sl.TrustRevisionType.SHARES, sl.ConflictType.AVERAGE, 2),
    sl.WeightedTypes(sl.RelationType.HARMONY, sl.TrustRevisionType.SHARES, sl.ConflictType.AVERAGE, 2),
]

fusion_conflict, t_vec_updated_conflict = sl.TrustedFusion.fuse_opinions_(sl.FusionType.CUMULATIVE, weighted_types_conflict, t_vec)
fusion_harmony, t_vec_updated_harmony = sl.TrustedFusion.fuse_opinions_(sl.FusionType.CUMULATIVE, weighted_types_harmony, t_vec)
...
for idx, t_op in enumerate(t_vec_updated_conflict):
    trust = t_op.trust
    name = f'RCTAS{idx}'
    trust_file.write(define_opinion_tikz_string.format(prior=trust.getBinomialPrior(), name=name, belief=trust.belief(), disbelief=trust.disbelief(), uncertainty=trust.uncertainty()))
    trust_file.write('\n')

for idx, t_op in enumerate(t_vec_updated_harmony):
    trust = t_op.trust
    name = f'RHTAS{idx}'
    trust_file.write(define_opinion_tikz_string.format(prior=trust.getBinomialPrior(), name=name, belief=trust.belief(), disbelief=trust.disbelief(), uncertainty=trust.uncertainty()))
    trust_file.write('\n')
...
fusion_ops = [normal_fusion, trusted_fusion, fusion_conflict, fusion_harmony]
fusion_names = ['FN', 'TF', 'TRC', 'TRH']
