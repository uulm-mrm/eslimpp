...
struct MultiSourceConflictLoader
{
  ...
  {
    bound_class.def_static("conflict",
                           nb::overload_cast<sl::ConflictType, ARGS...>(&slm::Conflict::template conflict<ARGS...>));
  ...
  static void load(::nanobind::class_<sl::multisource::Conflict>& nb_mod)
  {
    ...
    //    nb_mod.def_static(
    //        "conflict",
    //        nb::overload_cast<sl::ConflictType, std::vector<Opinion>, std::optional<std::vector<bool>>>(
    //            &slm::Conflict::template conflict<Opinion>));
    nb_mod.def_static("conflict", [](sl::ConflictType conflict_type, std::vector<Opinion> vec) -> FloatT {
      return slm::Conflict::conflict<Opinion>(conflict_type, vec);
    });
    nb_mod.def_static(
        "conflict_selective",
        [](sl::ConflictType conflict_type, std::vector<Opinion> vec, std::vector<bool> vec_bool) -> FloatT {
